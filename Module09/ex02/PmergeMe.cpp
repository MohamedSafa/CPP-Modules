...
void PmergeMe::parse_and_validate(int argc, char *argv[])
{
    ...
    for(int i = 1; i < argc ;i++)
    {
        ...
        this->_deq.push_back(static_cast<int>(nb));
    }
}
...
static std::vector<size_t> generateJacobSthal(size_t pendSize)
{
    std::vector<size_t> jacobSequence;
    jacobSequence.push_back(1);
    jacobSequence.push_back(3);
    while(jacobSequence.back() < pendSize)
    {
...
static std::vector<size_t> defineGroups(size_t pendSize, std::vector<size_t>& jacob)
{
    ...
    for(size_t j = 1; j < jacob.size(); j++)
    {
        ...
        size_t group_end   = std::min(jacob[j], pendSize);

        for(size_t x = group_end; x >= group_start; x--)
            order.push_back(x);

        if(group_end >= pendSize)
            break;
    }
    return order;
}

void PmergeMe::insertPendVec
(
    std::vector<int>& main,
    std::vector<int>& pend,
...
std::vector<int> PmergeMe::fordJohnsonVec(std::vector<int>& arr)
{
    ...
    new_main = fordJohnsonVec(main_v);
    ...
    std::vector<size_t> jacob = generateJacobSthal(new_pend.size());
    std::vector<size_t> insertionOrder = defineGroups(new_pend.size(), jacob);
    std::vector<int> sorted_winners_snapshot = new_main;

    insertPendVec(new_main, new_pend, sorted_winners_snapshot, insertionOrder, isodd);
    return new_main;
}



void PmergeMe::insertPendDeq
(
    std::deque<int>& main,
    std::deque<int>& pend,
...
)
{
    ...
    for(size_t i = 0; i < insertionOrder.size(); i++)
    {
        ...
        std::deque<int>::iterator winnerHi;
        bool isStraggler = (hasStraggler && idx == pend.size() - 1);
        if(isStraggler)
            winnerHi = main.end();
        else
        ...
        std::deque<int>::iterator groupHi;
        if(static_cast<size_t>(high) < main.size())
            groupHi = main.begin() + high;
        else
            groupHi = main.end();
        ...
        std::deque<int>::iterator lo = main.begin();
        while(lo < hi)
        {
            _compCount++;
...
std::deque<int> PmergeMe::fordJohnsonDeq(std::deque<int>& arr)
{
    ...
    std::vector<size_t> jacob = generateJacobSthal(new_pend.size());
    std::vector<size_t> insertionOrder = defineGroups(new_pend.size(), jacob);
    std::deque<int> sorted_winners_snapshot = new_main;

    insertPendDeq(new_main, new_pend, sorted_winners_snapshot, insertionOrder, isodd);
    return new_main;
}
